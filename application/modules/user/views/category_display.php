	...
	<div class="col-sm-12" >
			...
			<br>
			  <?php foreach($items as $m): ?><?php if($m->item_id!=0){?>
			  	<div class="col-sm-6">
			  	<?php if(isset($search)){?>
			  	<a href="<?php echo base_url().$this->session->userdata('logged_in')['user']; ?>/itemdetails/<?php echo $m->user_type.'/'.$m->item_id; ?>">
			  	<?php }else{?>
			    <a href="<?php echo base_url().$this->session->userdata('logged_in')['user']; ?>/itemdetails/<?php echo $user_type.'/'.$m->item_id; ?>">
			    <?php }?>
